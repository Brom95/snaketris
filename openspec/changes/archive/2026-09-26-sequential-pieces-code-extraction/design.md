...
Each `update()` computes the snake first (`moveSnake`), then the pieces (`stepPiece`). After the pieces step, `consumePieceAtHead()` eats a falling-piece cell that now coincides with the snake's **head** (so a block that falls onto the head is eaten); only the head eats — the body never consumes a piece. +1 score per cell, growing the snake one segment per cell. Rationale: with the old order (pieces first) a piece dropped ~0.84 cells per snake step, so the head aligned with the piece for only one step and the snake ate ~1 cell per piece. Snake-first lets the head consume the cells it passes through while keeping the snake strictly faster than the pieces.
