...
**D6. Tick order and eating (playtest fix).**
Each `update()` computes the snake first (`moveSnake`), then the pieces (`stepPiece`). After the pieces step, `consumePiecesOnSnake()` eats every falling-piece cell that now coincides with a snake segment (head or body), +1 score per cell, growing the snake one segment per cell. Rationale: with the old order (pieces first) a piece dropped ~0.84 cells per snake step, so the head aligned with the piece for only one step and the snake ate ~1 cell per piece. Snake-first plus "eat anything the snake overlaps" lets the snake consume the cells it passes through as well as any block that lands on its body, matching the intended feel while keeping the snake strictly faster than the pieces.
