...
- [x] 6.3 Fix spawn: `spawnPiece()` stored the shared `TETROMINOES` entry by reference, so eating (which splices the falling piece's `shape`) mutated the templates — pieces degenerated to a single block and then to an empty shape that never lands, stalling all spawns. Spawn now copies the shape; verify the table stays intact after eating and after a full game
